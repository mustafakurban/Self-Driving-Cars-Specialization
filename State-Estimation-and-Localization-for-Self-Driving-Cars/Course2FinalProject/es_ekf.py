...
p_cov[0] = np.zeros(9)  # covariance of estimate, for kalman filter
gnss_i  = 0
lidar_i = 0
...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    q_check = Quaternion(axis_angle = delta_x[6:]).quat_mult_left(q_check)
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    C_ns = Quaternion(*q_est[k-1]).to_mat() # asterix here is tuple unpacking.
    v_est[k] = v_est[k-1] + delta_t * (C_ns.dot(imu_f.data[k-1] + g))
    q_est[k] = Quaternion(euler = delta_t * imu_w.data[k - 1]).quat_mult_left(q_est[k - 1])
    p_est[k] = p_est[k-1] + (delta_t  * v_est[k-1]) + (pow(delta_t,2) / 2) * (C_ns.dot(imu_f.data[k-1] + g)) 

   
    # 1.1 Linearize the motion model and compute Jacobians

    predicted_x = np.zeros([9, 1])
    predicted_x[0][0] = p_est[k][0]
    predicted_x[1][0] = p_est[k][1]
    predicted_x[2][0] = p_est[k][2]
    predicted_x[3][0] = v_est[k][0]
    predicted_x[4][0] = v_est[k][1]
    predicted_x[5][0] = v_est[k][2]
    predicted_x[6][0] = var_imu_w
    predicted_x[7][0] = var_imu_w
    predicted_x[8][0] = var_imu_w
